...
/**
 * @description 喔噻支付宝支付策略类，实现此策略的具体逻辑
 * @author huqili
 * @date 2017年2月16日
 * @remark 此类是通过反射机制拿到的（相当于new出的对象）， 所以不能使用注解得到其对象。 此类下面的其它对象也不能使用注解拿到
 */
public class WosaiAlipayStrategy extends BaseStrategy implements IPayStrategy {

	@Override
	public PayResponVo pay(Order order, Merchandise merch, List<ResultBody> bodyList, String clientPackageName) 
	{
		logger.info("Enter wosai Alipay...");
		
		int returnCode = ReturnCode.SUCCESS.getCode();  //定义默认状态码
		String status = Order.STATUS_WAIT;  //定义默认支付状态
		
		//生成支付二维码（需要订单号，商品等信息）
		String QRCode = WosaiHandler.getQRCode(order, Order.PAYTYPE_WOSAI_ALIPAY);
		
		//封装响应参数
		bodyList = PayHandler.fillBodyList(bodyList, PayConfig.PAY_SPQ, PayHandler.getParam(QRCode, Order.PAYTYPE_WOSAI_ALIPAY));
