package com.newspace.aps.payChannel.unicomiptv.strategy;
 ...
 */
public class IptvStrategy  extends BaseStrategy implements IPayStrategy  {

	@Override
	public PayResponVo pay(Order order, Merchandise merch, List<ResultBody> bodyList, String clientPackageName) {
		
		logger.info("Enter Unicom IPTV...");
		
		int returnCode = ReturnCode.SUCCESS.getCode();  //定义默认状态码
		String status = Order.STATUS_WAIT;  //定义默认支付状态
		
		bodyList = PayHandler.fillBodyList(bodyList, PayConfig.PAY_CTS, PayHandler.getParamCTS(Order.PAYTYPE_IPTV));
